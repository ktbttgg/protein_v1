...
export function ResultsScreen({
  ...
  const heroTweak =
    ...
    (protein >= mealTarget
      ? "Keep this meal as-is. If you still want more volume, add extra vegetables."
      : "Add a quick protein top-up like Greek yoghurt, eggs, tuna or leftover chicken.")

  const watchouts = Array.isArray(result.watchouts)
    ? result.watchouts.slice(0, 2)
    : []
      ...
      <main className="flex flex-1 flex-col gap-6">
        ...
        <Card className="p-8">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="flex items-center gap-2 rounded-full bg-muted/50 px-4 py-2">
              ...
              <span
                className={`text-sm font-medium capitalize ${confidenceColor}`}
              >
        ...
        <Card className="p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div
              className={`rounded-full px-3 py-1 text-xs font-medium ${deltaTone}`}
            >
              {deltaLabel}
            </div>
        ...
        <Card className="p-6">
          ...
          <p className="text-xs text-muted-foreground">
            Not "bad" — just things that can quietly add up while chasing protein.
          </p>
